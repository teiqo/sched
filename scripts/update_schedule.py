...
Запуск: python3 scripts/update_schedule.py [локальный.pdf для проверки]
...
OUT = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))), "public", "data", "schedule.json")
...
def parse_pdf(path):
    ...
    with pdfplumber.open(path) as pdf:
        for page in pdf.pages:
            ...
            # заголовок с шифрами групп начинает новую секцию
    ...
    for code in sorted(cells, key=lambda c: c.lower()):
        ...
        for (day, bell), text in cells[code].items():
            ...
            for subject, teacher, room, extra in clean_cell(text):
                # Display preferences belong to the client, not the shared data.
